...
if strcmp(MAP_VAR_LIST.rectbox,'on') |  strcmp(MAP_VAR_LIST.rectbox,'circle'),
 ...
 % But sneakily we clip them in transforming, so we end up with isisfinite values only
 ...
 istart=sum(cumsum(isisfinite(X))==0)+1+[0:size(X,2)-1]*size(X,1);
 iend=size(X,1)-sum(cumsum(isisfinite(flipud(X)))==0)+[0:size(X,2)-1]*size(X,1);
